...
public class BlockingCallback implements Callback {
	...
	/*jetty默认的blocking callback没有设置超时*/
	/*可能导致在http流写回的时候,由于网络异常终端,导致线程死锁*/
	/*现在这种解决方式也不是很好,因为如果写回非常大的数据会导致数据没写完就释放锁了*/
	/*所以这里只适用于高吞吐量,但报文很小的service*/
	...
	public BlockingCallback() {
		LOG.debug("create timeout base blocking callback instance");
	}
	...
	public void block() throws IOException {
		...
		try {
			_latch.await(timeout, TimeUnit.SECONDS);
